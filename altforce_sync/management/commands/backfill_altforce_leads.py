# altforce_sync/management/commands/backfill_altforce_leads.py
...
def _parse_any_dt(s: str) -> datetime:
    ...
    Aceita:
      ...
      - BR:     07/10/2025, 07/10/2025 12:34, 07/10/2025 12:34:56
...
def _fmt_dt(dt: datetime) -> str:
    # Apenas para logs com microssegundos (como nos demais comandos de backfill)
    return dt.strftime("%Y-%m-%d %H:%M:%S.%f")


class Command(BaseCommand):
    help = "Backfill de /leads em janelas (padrão 30 dias) com guardas contra loop."
    ...
    def handle(self, *args, **opts):
        ...
        while True:
            ...
            # Calcula fim da janela: start + N dias - 1 µs, limitado a to_dt
            ...
            # Failsafe: se repetiu exatamente a mesma janela, encerra
            ...
            # --- chama o serviço ---
            try:
                summary = sync_leads(start_dt=current_start, end_dt=current_end)
            ...
            # 'errors' normalmente é lista; mantém fallback numérico se vier de outro jeito
            errs = summary.get("errors", 0)
            if isinstance(errs, list):
                errors_n = len(errs)
            else:
                try:
                    errors_n = int(errs or 0)
                except Exception:
                    errors_n = 0

            self.stdout.write(self.style.SUCCESS(
                f"   ✓ OK — recebidos={received}, criados={created}, "
                f"atualizados={updated}, erros={errors_n}"
            ))
        ...
        self.stdout.write(self.style.SUCCESS("Backfill de leads finalizado."))
